...
if ( !function_exists('tatsu_video') ) {
	function tatsu_video( $atts, $content ) {
	    ...
	    switch ( $source ) {
			...
			default:
				$output .= ( isset( $animate ) && 1 == $animate ) ? '<div class="tatsu-animate" data-animation="'.$animation_type.'">' : '' ; 
				$output .= '<div class="tatsu-module tatsu-video tatsu-hosted-wrap"><video width = "100%" controls controlsList="nodownload" poster = "'.$placeholder.'"><source src="'.$url.'" type="video/mp4"></video></div>';
				$output .= ( isset( $animate ) && 1 == $animate ) ? '</div>' : '' ;
